...
module FaceRenderer {
    ...
    //
    function drawCachedTightText(dc as $.Toybox.Graphics.Dc, x as $.Toybox.Lang.Number, y as $.Toybox.Lang.Number, font as $.Toybox.Graphics.FontDefinition or $.Toybox.Graphics.VectorFont, charStrings as $.Toybox.Lang.Array<$.Toybox.Lang.String>, widths as $.Toybox.Lang.Array<$.Toybox.Lang.Number>, totalW as $.Toybox.Lang.Number, tracking as $.Toybox.Lang.Number, debug as $.Toybox.Lang.Boolean, mainColor as $.Toybox.Graphics.ColorValue, fontH as $.Toybox.Lang.Number) as Void {
        var curX = (x - (totalW / 2)) as $.Toybox.Lang.Number;

        dc.setColor(mainColor, $.Toybox.Graphics.COLOR_TRANSPARENT);

        for (var i = 0; i < charStrings.size(); i++) {
            var s = charStrings[i];
            if (debug) {
    ...
    function drawRingArc(dc as $.Toybox.Graphics.Dc, radius as $.Toybox.Lang.Number, ratio as $.Toybox.Lang.Float, color as $.Toybox.Graphics.ColorValue, cx as $.Toybox.Lang.Number, cy as $.Toybox.Lang.Number, width as $.Toybox.Lang.Number) as Void {
        ...
        dc.drawArc(cx, cy, radius, $.Toybox.Graphics.ARC_COUNTER_CLOCKWISE, start, currentEnd.toNumber());
    }
